...
export default function ComponenteCSeleccionManualConfirmar({ cambiarVista }: HijoProps) {
  const { selectedTickets, toggleTicket, totalBs, setMode, socialLinks } = usePurchase();
  ...
  <div className="mt-8 md:mt-24">
    <div className="w-full max-w-lg md:max-w-6xl mx-auto p-4 animate-in fade-in slide-in-from-bottom-8 duration-500">
      ...
      <div className="mb-8 max-w-md mx-auto">
        <button className="w-full py-8 border-2 border-dashed border-slate-700 hover:border-[var(--color-primary)] rounded-2xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:text-white transition-all bg-slate-900/40 group"
          ...
          <span className="font-bold">
            {selectedTickets.length > 0 ? "Editar mis números" : "Elegir mis números"}
          </span>
        </button>
        <div className="mt-4 flex flex-wrap gap-2 justify-center">
          {selectedTickets.length === 0 && (
            <span className="text-xs text-slate-500">Aún no has elegido números.</span>
          )}
          {selectedTickets.map((t) => (
            <button
              ...
              title="Quitar"
              className="bg-[var(--color-primary)] text-black text-xs font-black px-3 py-1.5 rounded shadow-sm flex items-center gap-1 hover:brightness-110"
            >
              {t}
      ...
      <div className="flex items-stretch gap-3 h-[72px] max-w-2xl mx-auto mb-4">
        <button className="flex-1 bg-[var(--color-primary)] rounded-2xl flex items-center justify-between px-6 transition-all active:scale-[0.98] shadow-[0_0_30px_rgba(var(--color-primary-rgb),0.3)] text-[var(--color-primary-foreground)] group hover:brightness-110 relative overflow-hidden disabled:opacity-50"
          ...
          <div className="flex flex-col items-start z-10 py-1 min-w-0">
            <span className="text-xs font-bold uppercase tracking-wider opacity-80 mb-0.5">
              Participar ({selectedTickets.length})
            </span>
